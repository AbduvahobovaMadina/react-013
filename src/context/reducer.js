  ...
  switch (action.type) {
    ...
    case "TOGGLE_WISHLIST":
      ...
      if(index < 0){
        ...
        memory =  {...state, wishlist: state.wishlist.filter(item => item.id !== action.payload.id)}
        saveStorege(memory)
        return memory
      }
    case "INC_CART":
      ...
      if(cartIndex < 0){
        memory = {...state, cart: [...state.cart, {...action.payload, quantity: 1}]}
        saveStorege(memory)
        return memory
      }else{
        memory = {...state, cart: state.cart.map(item => item.id === action.payload.id ? {...item, quantity: item.quantity + 1} : item)}
        saveStorege(memory)
        return memory
      }
    case "DEC_CART":
